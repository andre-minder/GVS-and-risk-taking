...
# same as above
GDT_files <- list.files(path = "data_test/", recursive = TRUE, pattern = "*gameofdicetask_run*")

GDT_raw <- map_dfr(.x = set_names(paste0("data_test/", GDT_files)),
                   .f = ~ read_tsv(.x, col_types = cols(.default = "?", frameRate = "c")))
...
In this section, the number of safe/risky choices per stimulation block and participant is calculated. The additional net score of the GDT is calculated in the following way: Sum of save choices (i.e. safest plus safe option) - sum of risky choices (i.e. risky plus riskiest option). A positive score thus hints at a safe player, while a negative score indicates risky playing.
...
GDT_scored <- GDT_clean_with_conditions |> 
  ...
  rowwise() |> 
  mutate(score = sum(safest, safe, na.rm = TRUE) - sum(risky, riskiest, na.rm = TRUE)) |> 
  select("subject", "session", "stimulation", everything())
...
write_csv(BART_all, file = "data/processed/BART_data_processed.csv")
write_csv(GDT_all, file = "data/processed/GDT_data_processed.csv")
